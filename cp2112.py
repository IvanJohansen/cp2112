...
class CP2112:
    ...
    """
    def __init__(self, clock=100000, serial_number=None, retry=0):
        ...
        self.handle.send_feature_report(
            ...
            clock >> 24, (clock >> 16) & 0xFF, (clock >> 8) & 0xFF, clock & 0xFF, # Clock in Hz
            0x00, # Device address (disabled)
            0x00, # Auto Send Read (disabled)
            0x00, 0x00, # Write Timeout (No Timeout)
            0x00, 0x00, # Read Timeout (No timeout)
            0x00, # SCL Low Timeout (disabled)
            (retry >> 8) & 0xFF, retry & 0xFF]) # Retry Time (number of retry attempts)

        self.set_gpio_config(0x03, 0x03, 0x03, 0)
    ...
    def set_gpio_config(self, dir, push_pull, special, clock_divider):
        ...
        """
        self.handle.send_feature_report([dir, push_pull, special, clock_divider])
    ...
    def write_read_i2c(self, address, tx_data, rx_size):
        ...
        if not 1 <= len(tx_data) <= 16 or not 1 <= rx_size <= 512:
            raise IndexError()
        self.handle.write(bytes([0x11, address << 1, rx_size >> 8, rx_size & 0xFF, len(tx_data)]) + bytes(tx_data))
        count = self._wait_transfer_finish()
        assert count == rx_size
        self.handle.write([0x12, rx_size])
        response = self._wait_response(0x13, rx_size + 3)
